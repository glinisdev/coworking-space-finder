...
export const strengthColor = (count: number) => {
    if (count < 2) return { label: 'Плохой', color: 'error.main' };
    if (count < 3) return { label: 'Слабый', color: 'warning.main' };
    if (count < 4) return { label: 'Нормальный', color: 'warning.dark' };
    if (count < 5) return { label: 'Хороший', color: 'success.main' };
    if (count < 6) return { label: 'Безопасный', color: 'success.dark' };
    return { label: 'Плохой', color: 'error.main' };
};
